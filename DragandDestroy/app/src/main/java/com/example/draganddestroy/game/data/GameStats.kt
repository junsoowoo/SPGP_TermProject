...
object GameStats {
    ...
    const val DEBUG_TEST_CLEAR_MODE = true

    val playerDamage: Int
        get() = if (DEBUG_TEST_CLEAR_MODE) 80 else 4 + (playerDamageLevel - 1) * 2

    val playerMaxHp: Int
        get() = if (DEBUG_TEST_CLEAR_MODE) 9999 else 135 + (playerMaxHpLevel - 1) * 35
    ...
    val playerFireInterval: Float
        get() = if (DEBUG_TEST_CLEAR_MODE) 0.035f else (0.08f - (playerFireRateLevel - 1) * 0.007f).coerceAtLeast(0.045f)
    ...
    fun getBasicTurretDamage(): Int {
        return if (DEBUG_TEST_CLEAR_MODE) 650 else 72 + turretDamageBonus
    }

    fun getRapidTurretDamage(): Int {
        return if (DEBUG_TEST_CLEAR_MODE) 180 else 17 + turretDamageBonus / 2
    }

    fun getBasicTurretHp(): Int {
        return if (DEBUG_TEST_CLEAR_MODE) 9999 else 125 + turretHpBonus
    }

    fun getRapidTurretHp(): Int {
        return if (DEBUG_TEST_CLEAR_MODE) 9999 else 85 + turretHpBonus
    }

    fun getBasicTurretRange(): Float {
        return if (DEBUG_TEST_CLEAR_MODE) 430f else 290f + turretRangeBonus
    }

    fun getRapidTurretRange(): Float {
        return if (DEBUG_TEST_CLEAR_MODE) 390f else 240f + turretRangeBonus
    }
